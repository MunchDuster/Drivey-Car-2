...
public class NeuralNetwork : IComparable<NeuralNetwork>
{
	...
	private void InitBiases()
	{
		...
		for (int i = 1; i < layers.Length; i++) //run through all layers
		{
	...
	public float[] FeedForward(float[] inputs)
	{
		...
		{
			neurons[0][j] = sigmoid(inputs[j]);
		}

		//iterate over all neurons and compute feedforward values 
		for (int l = 1; l < layers.Length; l++)//index of layer - l
		{
			for (int i = 0; i < layers[l]; i++)//index in layer - i
			{
				float value = biases[l - 1][i];

				for (int li = 0; li < layers[l - 1]; li++)//index in last layer - li
				{
					value += weights[l - 1][i][li] * neurons[l - 1][li]; //sum off all weights connections of this neuron weight their values in previous layer
				}

				neurons[l][i] = sigmoid(value);
			}
		}
